...
void J2534::dbgprint(char* Format, ...)
{
	va_list arglist;
	int cb;
...
void J2534::dbgdump(unsigned char *data,unsigned int datalen,int kind)
{
	unsigned int i;
...
void J2534::dbgprintptmsg(PASSTHRU_MSG *pMsg,int kind)
{
	if (!pMsg)
	{
...
long J2534::LoadJ2534DLL(char* szDLL)
{
...
long J2534::PassThruOpen(void *pName, unsigned long *pDeviceID)
{
...
long J2534::PassThruWriteMsgs(unsigned long ChannelID, PASSTHRU_MSG *pMsg, unsigned long *pNumMsgs, unsigned long Timeout)
{
	unsigned int i;
	long result = STATUS_NOERROR;
	if (!checkDLL())
...
long J2534::PassThruStartPeriodicMsg(unsigned long ChannelID, PASSTHRU_MSG *pMsg, unsigned long *pMsgID, unsigned long TimeInterval)
{
	long result = STATUS_NOERROR;
	if (!checkDLL())
		return ERR_DEVICE_NOT_CONNECTED;
...
void J2534::dump_sbyte_array(SBYTE_ARRAY* s)
{
...
void J2534::dump_sconfig_param(SCONFIG s)
{
	...
	case INPUT_RANGE_HIGH:
		...
		break;
	default:
		sprintf(paramName,"%u(unknown)",s.Parameter);
		break;
	}
...
long J2534::PassThruIoctl(unsigned long ChannelID, unsigned long IoctlID,
                      void *pInput, void *pOutput)
{
	...
	case TX_IOCTL_APP_SERVICE:
		...
		break;
	default:
		sprintf(IoctlName,"%u(unknown)",IoctlID);
		break;
	}
